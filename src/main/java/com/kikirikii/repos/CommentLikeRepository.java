...
public interface CommentLikeRepository extends CrudRepository<CommentLike, Long> {
    ...
    @Query("select l from CommentLike l where l.comment.id = :commentId and l.state='ACTIVE'")
    List<CommentLike> findAllActiveByCommentId(@Param("commentId") Long commentId);
}
